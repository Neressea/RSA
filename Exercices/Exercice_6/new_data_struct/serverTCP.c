/*Ici, on a reprit le code d'une serveur TCP IPv4 et on a modifie les structures IPv4 pour des structure IPv6*/
#include <sys/types.h>
...

{
  ...
  if (bind(serverSocket,(struct sockaddr *)&serv_addr6, sizeof(serv_addr6) ) <0) {
  ...
  clientSocket = accept(serverSocket, (struct sockaddr *) &cli_addr6, &clilen);
  if (clientSocket < 0) {
