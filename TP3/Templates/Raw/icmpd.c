...

{
  ...
  for (;;) {
    ...
    lenIPHeader = ip->ip_hl * 4 ;       // ip->ip_hl longueur  en mot de 32 bits -> on passe en octets
  
    icmp =  (struct icmp *) (rcvbuffer + lenIPHeader); // debut entete ICMP : on met les parenthèses pour que le décalage de pointeur soit pour des char
    ip2 =  (struct ip *) (rcvbuffer + lenIPHeader + 8); // debut en-tete IP contenu dans ICMP : pareil, on décale les char
    lenIPHeader2 = ip2->ip_hl * 4; // longueur  en-tete IP
    if (ip2->ip_p == IPPROTO_UDP) {
      udp = (struct udphdr *) ((char *) ip2 + lenIPHeader2) ; // debut en-tete UDP
